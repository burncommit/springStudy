...
import me.angukbum.springbootdeveloper.dto.ArticleResponse;
...
public class BlogApiController {
    ...
    public ResponseEntity<Article> addArticle(@RequestBody AddArticleRequest request){
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(savedAriticle); //[응답코드]로 201 즉 created를 응답하고 테이블에 저장된 객체 반환
    }
    ...
    public ResponseEntity<List<ArticleResponse>> findAllArticles(){
        List<ArticleResponse> articles = blogService.findAll()
                .stream()
                .map(ArticleResponse::new) //blogService.findAll()은 반환형이 Article인데 ArticleResponse로 변경한거임
                .toList();
